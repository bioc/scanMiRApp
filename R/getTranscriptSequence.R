                           ...
                           label_notes = FALSE, verbose = TRUE, ...) 
{
  ...
  if ((is(miRNA, "character") && length(miRNA) == 1 && nchar(gsub("[ACGTU]", 
                                                                  "", miRNA)) == 0) || is(miRNA, "KdModel")) {
    mods <- miRNA
    if(!(is(miRNA,"KdModel"))){
    ...
    mods <- mods[[miRNA]]
  } else if (length(w <- grep(miRNA, names(mods), ignore.case = TRUE)) == 
             1) {
    mods <- mods[[w]]
  } else {
  ...
  if(is(miRNA, "KdModel")){
    ...
    p <- ggplot(m, aes(x = start, y = -logKd)) + 
      ...
      labs(x = "sequence length", y = bquote("-"*logK[d]), title = title) +  
